...
import { useDeleteStudCertificate, useFetchStudCertificate } from "./hooks/useStudCertificate";
...
export default function StudCertificate() {
    ...
    const { loading, error, studCertificate, setSelectedStudCertificate } = useFetchStudCertificate();
    const { deleteStudCertificate, loading: deleteLoading, error: deleteError } = useDeleteStudCertificate(); // Added this hook
    ...
    const handleDeleteClick = async (id: string | number) => {
        const confirmDelete = window.confirm("Are you sure you want to delete this stud certificate?");
        if (confirmDelete) {
            try {
                    ...
                    <TableBody>
                        {paginatedData.map((order, index) => (
                            <TableRow key={order.id} className={index % 2 === 0 ? "bg-gray-100 dark:bg-gray-800" : "bg-white dark:bg-gray-900"}>
                                ...
                                <TableCell className="px-5 py-4 text-start">{order?.sire?.dogName}</TableCell>
                                <TableCell className="px-5 py-4 text-start">{order?.dam?.dogName}</TableCell>
                                <TableCell className="px-5 py-4 text-start">{formatDate(order?.matingDate)}</TableCell>
                                ...
                                <TableCell className="px-4 py-3 text-start">
                                    <button className="text-blue-500 mx-1" onClick={() => openViewModal(order)}>
                                        <EyeIcon />
                                    </button>
                                    ...
                                    <Tooltip title="Remove">
                                        <button className="text-red-500 mx-1" onClick={() => handleDeleteClick(order.id)} disabled={deleteLoading} >
                                            {deleteLoading ? (
